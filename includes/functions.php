...
function process_specials($include, $file_contents) {
	...
	// e.g., ["markdown/ext/06", "1-sdlc", "md"]
	$include_dotsplit  = explode(".", $include);
	// e.g., "md"
	$include_extension = end($include_dotsplit);
		...
		if ($include_category == "code") {
			...
			foreach ($tab_indents[0] as &$tab_indent) {
				...
				$space_count = $space_count < 4 ?
					$space_count + substr_count($tab_indent, "\\n") :
					$space_count;
			...
			// indent each line with a tab for markdown
			$file_contents = preg_replace("/(.*\n)/", "\t$1", $file_contents);

		} else if ($include_extension == "html") {

			preg_match("/<body[A-z0-9='.,:; \"]*>((?s).*?)<\/body>/",
				$file_contents,
				$file_contents);
			$file_contents = $file_contents[1];
			// prevents markdown from converting markup to HTML entities
			$file_contents = preg_replace("/[\n\r\t]*/", "", $file_contents);
	...
	if ($include_extension == 'md') {
		...
		if (isset($include_template[0])) {
			...
			if ($include_template == 'article') {
				...
				$file_contents = str_replace('<!--[COL-1]-->',
					'<div class="col-1">',
					$file_contents);
				$file_contents = str_replace('<!--[COL-2]-->',
					'</div><div class="col-2">',
					$file_contents);
				$file_contents = $file_contents . '</div>';
				$file_contents = '<hr><article>' .
					$file_contents .
					'</article><hr>';
			...
			if (strlen($second_to_last) > 12 &&
				...
				$file_contents = '<div class="n"><span class="hb">Blue highli' .
					'ghting</span> indicates new passages of text.</div>' .
					$file_contents;
			}
...
function process_includes($markdown) {
	...
	preg_match_all('/(?<=<!--\[INCLUDE\] )[A-z0-9\-_\/.]*.' .
		'[md|txt|html|vba|conf|py|c](?= -->)/', $markdown, $includes);
	...
	foreach ($includes[0] as $include) {
		...
		if (file_exists($include)) {
			...
			$markdown = preg_replace("/<!--\[INCLUDE\] $include_quote -->/",
				$file_contents,
				$markdown);
