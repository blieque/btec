...
Along the top of the interface are large icons allowing the user to quickly access basic tools, render the scene immediately in the editor viewport, and add the various building blocks to the scene. The objects are grouped into a number of different categories: **primitive shapes** such as cubes and cylinders; **primitive splines** (paths) such as circles and freehand curves; **NURBS** used to create and modify objects given other shapes; **arrays** and other mechanisms; **deformers** for actions such as twisting and pillowing objects; **environment objects** including infinite floors and skies; **cameras** for capturing complex shots; and **lights** for lighting the scene in many different ways. ***QUICK + EASY***

In the second animation I used a good deal of tools and functionality of Cinema, while still only scratching the surface of what it is capable of. Some of the most prominent items I used are detailed below.

As a first step in creating the animation, I build the basic scene using the primitive and environment object categories. To begin I added the main sphere in the centre, and then the seamless floor. I added another sphere, likely by duplicating the pre-existing one in the Objects panel in the top right, and scaled it to the correct size using the Attributes panel in the bottom right. Using a cloner object from the MoGraph toolkit, I duplicated the smaller sphere in a 10 &times; 1 &times; 10 grid, and positioned the array above the larger sphere. MoGraph cloners can be used to duplicate a huge variety of objects, including shapes, lights and other MoGraph objects, in many different manners. For example, the clones can be arranged in a grid, in a line, radially or to fill the volume of another object. MoGraph also includes Effectors, such as the random effector, which can be used to very quickly vary the sizes of copies in a MoGraph cloner. ***SIMPLE***

Dynamics &mdash; *moar mograph*  
very easy, very good, different shape modes, activation, parameters, imperfect with intersection, soft-bodies iffy

Modelling the lights  
easy to find, different types, slowness of area shadows, good looking

Texturing  
different effects, Fresnel, limited caustics (sshhh..), big impact on success of animation, 

Keyframing  
v. v. easy, interpolation -> smooth animation, f-curves, could do with more curve presets

Camera  
easy to use, focal distance, field of view, parallel/isometric -> appearance, rotation around null
